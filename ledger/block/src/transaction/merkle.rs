...
impl<N: Network> Transaction<N> {
    ...
    pub fn deployment_tree(deployment: &Deployment<N>) -> Result<DeploymentTree<N>> {
        ...
        // Note: `ConsensusVersion::V7` requires the program checksum and program owner to be present, while prior versions require it to be absent.
        //   `Deployment::version` checks that this is the case.
        // Note: After `ConsensusVersion::V7`, the program checksum and owner are used in the header of the hash instead of the program ID.
