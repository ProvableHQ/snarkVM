...
macro_rules! insert_testnet_credit_keys {
    ($map:ident, $type:ident<$network:ident>, $variant:ident) => {{
        paste::paste! {
            ...
            $crate::insert_testnet_credit_v1_keys!($map, $type<$network>, $variant);
            $crate::insert_testnet_key!($map, string, $type<$network>, ("redelegate", $crate::testnet::[<Redelegate $variant>]::load_bytes()));
        }
    }};
}

#[macro_export]
macro_rules! insert_testnet_credit_v1_keys {
    ($map:ident, $type:ident<$network:ident>, $variant:ident) => {{
        paste::paste! {
            let string = stringify!([<$variant:lower>]);
