...
impl<A: Aleo> Response<A> {
    /// Returns the injected circuit outputs from a static call, given the number of inputs, tvk, tcm, outputs, and output types.
    pub fn process_outputs_from_static_callback(
        network_id: &U16<A>,
                ...
                match output_types {
                    ...
                    console::ValueType::Future(..) => {
                        ...
                        match &output {
                            ...
                            Value::DynamicFuture(..) => A::halt("Expected a future output, found a dynamic future"),
                        }
                    }
                    // A dynamic future cannot be returned directly.
                    console::ValueType::DynamicFuture => {
                        A::halt("A dynamic future cannot be returned from a static call")
                    }
                }
            })
...
mod tests {
    ...
    // TODO (@d0cd) Testing for dynamic calls.
    ...
    fn check_from_callback(
        ...
        for i in 0..ITERATIONS {
            ...
            Circuit::scope(format!("Response {i}"), || {
                let outputs = Response::process_outputs_from_static_callback(
                    &network_id,
                    &program_id,
                    &function_name,
                    4,
