...
public:
  ...
  RustError NTT(fr_t *out, fr_t *in, uint32_t lg_domain_size,
    ...
    // // DEBUG: Print input to NTT::Base (CPU path via snarkvm_ntt)
    // if (ntt_direction == NTT::Direction::inverse && lg_domain_size == 15) {
    //   printf("CPU NTT::Base input (first 5 elements as hex):\n");
    //   for (int i = 0; i < 5; i++) {
    //     uint8_t bytes[32];
    //     memcpy(bytes, &h_mem[i], 32);
    //     printf("  [%d]: ", i);
    //     for (int j = 0; j < 32; j++)
    //       printf("%02x", bytes[j]);
    //     printf("\n");
    ...
    //   printf("CPU NTT::Base input (last 5 elements as hex):\n");
    //   for (int i = 0; i < 5; i++) {
    //     uint8_t bytes[32];
    //     memcpy(bytes, &h_mem[domain_size - 5 + i], 32);
    //     printf("  [%d]: ", i);
    //     for (int j = 0; j < 32; j++)
    //       printf("%02x", bytes[j]);
    //     printf("\n");
    ...
    // Perform NTT
    RustError e = Polynomial::Base(gpu, h_mem, lg_domain_size, ntt_order,
    ...
    // // DEBUG: Print output from NTT::Base (CPU path via snarkvm_ntt)
    // if (ntt_direction == NTT::Direction::inverse && lg_domain_size == 15) {
    //   printf("CPU NTT::Base output (first 5 elements as hex):\n");
    //   for (int i = 0; i < 5; i++) {
    //     uint8_t bytes[32];
    //     memcpy(bytes, &h_mem[i], 32);
    //     printf("  [%d]: ", i);
  ...
  RustError PolyMul(fr_t *out, size_t pcount, fr_t **polynomials, size_t *plens,
    ...
    if (pcount + ecount == 0) {
      ...
      if (pcount == 1) {
        ...
        return NTT(out, out, lg_domain_size, NTT::InputOutputOrder::NN,
    ...
    RustError e = Polynomial::Mul(
        gpu, stream, h_mem0, h_mem1, h_mem2, d_mem0, d_mem1, d_mem2, d_mem3,
        pcount, polynomials, plens, ecount, evaluations, elens, lg_domain_size);
    if (e.code != cudaSuccess) {
      resources.send(resource);
      return e;
    }
  ...
  RustError MSM(point_t *out, const affine_t points[], size_t npoints,
    ...
    // printf("snarkvm.cu MSM: npoints=%zu, ngpus=%zu, ffi_affine_size=%zu, "
    //        "sizeof(affine_t)=%zu\n",
    //        npoints, ngpus(), ffi_affine_size, sizeof(affine_t));
    ...
    for (size_t i = 0; i < gpu_count; i++) {
      pool.spawn([&, i]() {
        ...
        const affine_t *pts =
            (affine_t *)(&((uint8_t *)points)[start * ffi_affine_size]);
