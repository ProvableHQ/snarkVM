    ...
    D: Deserializer<'de>,
{
    ...
    let obj =
        T::deserialize_compressed(&mut reader).map_err(|e| D::Error::custom(format!("while deserializing: {e}")))?;
    if reader.is_empty() { Ok(obj) } else { Err(D::Error::custom("while deserializing: trailing bytes")) }
}
