...
impl<N: Network> Package<N> {
    ...
    pub fn get_process(&self) -> Result<Process<N>> {
        self.get_process_with_consensus_version(Arc::new(AtomicU16::new(ConsensusVersion::latest().to_u16())))
    }

    /// Returns a new process for the package.
    pub fn get_process_with_consensus_version(&self, consensus_version: Arc<AtomicU16>) -> Result<Process<N>> {
        // Load the default process.
        let mut process = Process::load_with_consensus_version(consensus_version)?;
