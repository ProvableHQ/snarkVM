...
use crate::{Assignments, CallStack, Process, Stack, StackExecute};
...
use console::{
    ...
    program::{Request, Identifier, ProgramID, Value},
};
...
fn transfer_private() {
    ...
    prepare_check_deployment::<_, CurrentAleo>(stack, &private_key, function_name, &[r0, r1, r2], rng);
}
...
fn transfer_public() {
    ...
    // Compute the assignment.
    prepare_check_deployment::<_, CurrentAleo>(stack, &private_key, function_name, &[r0, r1], rng);
}
...
function do:
    ...
    // Compute the assignment.
    prepare_check_deployment::<_, CurrentAleo>(&stack, &private_key, function_name, &[r0], rng);
}
