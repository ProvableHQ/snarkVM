...
impl<N: Network> RegistersLoad<N> for ConstructorRegisters<N> {
    ...
    fn load(&self, stack: &(impl StackMatches<N> + StackProgram<N>), operand: &Operand<N>) -> Result<Value<N>> {
        ...
        match (self.constructor_types.get_type(stack, register), &value) {
            ...
            // Ensure the load is valid in a constructor context.
            (Ok(_type), stack_value) => bail!(
                "Attempted to load a '{stack_value}' value from a register '{register}' of type '{_type}' in a constructor",
            ),
