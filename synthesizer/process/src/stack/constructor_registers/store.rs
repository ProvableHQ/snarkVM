...
impl<N: Network> RegistersStore<N> for ConstructorRegisters<N> {
    ...
    fn store(
        ...
        match (register, stack_value) {
            (Register::Locator(locator), stack_value) => {
                ...
                match (self.constructor_types.get_type(stack, register), &stack_value) {
                    // Ensure the plaintext value matches the plaintext type.
                    (Ok(plaintext_type), Value::Plaintext(plaintext_value)) => {
                        stack.matches_plaintext(plaintext_value, &plaintext_type)?
                    }
                    // Ensure the store is valid in a constructor context.
                    (Ok(type_), stack_value) => bail!(
                        "Attempted to store a '{stack_value}' value in a register '{register}' of type '{type_}' in a constructor scope",
                    ),
