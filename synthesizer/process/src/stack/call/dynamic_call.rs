...
use circuit::Eject;
use console::prelude::FromField;
use console::program::{Identifier, Literal, Plaintext, ProgramID};
...
impl<N: Network> CallTrait<N> for DynamicCall<N> {
    ...
    fn execute<A: circuit::Aleo<Network = N>, R: Rng + CryptoRng>(
        ...
        // If the operator is a function, retrieve the function and compute the output.
            ...
            let program_id = circuit::ProgramID::new(circuit::Mode::Public, *substack.program_id());
            // Inject the function name as `Mode::Public`.
            let function_name = circuit::Identifier::new(circuit::Mode::Public, *function.name());
