...
impl<N: Network> Stack<N> {
    ...
    #[inline]
    pub fn check_upgrade_is_valid(old_program: &Program<N>, new_program: &Program<N>) -> Result<(), StackInitError> {
        ...
        if program_id == &ProgramID::from_str("credits.aleo").unwrap() {
            return Err(StackInitError::CreditsUpgrade);
        }
        // Ensure the program ID matches.
        if old_program.id() != new_program.id() {
            return Err(StackInitError::UpgradeDifferentProgramId(old_program.id().to_string()));
        }
        ...
        for old_import in old_program.imports().keys() {
            if !new_program.contains_import(old_import) {
                return Err(StackInitError::UpgradeMissingOriginalImport(
        ...
        match (old_program.constructor(), new_program.constructor()) {
            (_, None) => {
                return Err(StackInitError::UpgradeConstructorMissingOriginal);
            }
            (None, _) => {
                return Err(StackInitError::UpgradeConstructorMissingFinal);
            }
            (Some(old_constructor), Some(new_constructor)) => {
                if old_constructor != new_constructor {
                    return Err(StackInitError::UpgradeConstructorMismatch(old_program.id().to_string()));
                }
            }
        }
        ...
        for (old_mapping_id, old_mapping_type) in old_program.mappings() {
            let new_mapping_type = new_program.get_mapping(old_mapping_id).map_err(|_| {
                StackInitError::UpgradeMappingMissing(old_program.id().to_string(), old_mapping_id.to_string())
            })?;
            if *old_mapping_type != new_mapping_type {
                return Err(StackInitError::UpgradeMappingMismatch(
        ...
        for (old_struct_id, old_struct_type) in old_program.structs() {
            let new_struct_type = new_program.get_struct(old_struct_id).map_err(|_| {
                StackInitError::UpgradeStructMissing(old_program.id().to_string(), old_struct_id.to_string())
            })?;
            if old_struct_type != new_struct_type {
                return Err(StackInitError::UpgradeStructMismatch(
                    old_program.id().to_string(),
        ...
        for (old_record_id, old_record_type) in old_program.records() {
            let new_record_type = new_program.get_record(old_record_id).map_err(|_| {
                StackInitError::UpgradeRecordMissing(old_program.id().to_string(), old_record_id.to_string())
            })?;
            if old_record_type != new_record_type {
                return Err(StackInitError::UpgradeRecordMismatch(
                    old_program.id().to_string(),
        ...
        for old_closure in old_program.closures().values() {
            ...
            let new_closure = new_program.get_closure(old_closure_name).map_err(|_| {
                StackInitError::UpgradeClosureMissing(old_program.id().to_string(), old_closure_name.to_string())
            })?;
            if old_closure != &new_closure {
                return Err(StackInitError::UpgradeClosureMismatch(
                    old_program.id().to_string(),
        ...
        for old_function in old_program.functions().values() {
            ...
            let new_function = new_program.get_function_ref(old_function_name).map_err(|_| {
                StackInitError::UpgradeFunctionMissing(old_program.id().to_string(), old_function_name.to_string())
            })?;
            if old_function.input_types() != new_function.input_types() {
                return Err(StackInitError::UpgradeFunctionInputMismatch(
                    old_program.id().to_string(),
                    old_function_name.to_string(),
                ));
            }
            if old_function.output_types() != new_function.output_types() {
                return Err(StackInitError::UpgradeFunctionOutputMismatch(
                    old_program.id().to_string(),
                    old_function_name.to_string(),
                ));
            }
            match (old_function.finalize_logic(), new_function.finalize_logic()) {
                ...
                (None, Some(_)) => {
                    return Err(StackInitError::UpgradeFunctionFinalizeBlockUnexpected(
                        old_program.id().to_string(),
                        old_function_name.to_string(),
                    ));
                }
                (Some(_), None) => {
                    return Err(StackInitError::UpgradeFunctionFinalizeBlockExpected(
                        old_program.id().to_string(),
                        old_function_name.to_string(),
                    ));
                }
                (Some(old_finalize), Some(new_finalize)) => {
                    if old_finalize.input_types() != new_finalize.input_types() {
                        return Err(StackInitError::UpgradeFunctionFinalizeInputMismatch(
                            old_program.id().to_string(),
                            old_function_name.to_string(),
                        ));
                    }
