...
function fourth:
    ...
    let records = block.transactions().records().map(|(cm, r)| r.clone().decrypt(&caller_view_key).map(|r| (*cm, r))).collect::<Result<Vec<_>>>().unwrap();
    assert_eq!(records.len(), 4, "Expected three records to be minted.");
    ...
    assert!(deployment.is_err(), "Expected an error when deploying a program that returns a local record as an external record.");
...
function cannot_be_called_from_test_one:
    ...
    let record_value = records.next().unwrap().1;
    // let record_commitment = record_value.to_commitment(&ProgramID::from_str("test_one.aleo").unwrap(), &Identifier::from_str("First").unwrap(), &Field::from_str("0field").unwrap()).unwrap();
    ...
    // TODO: it seems like this call fails.
    let execution = vm
        .execute(
            &caller_private_key,
            ("test_one.aleo", "third"),
            vec![Value::Record(record_value)].into_iter(),
            None,
            0,
            None,
            rng,
