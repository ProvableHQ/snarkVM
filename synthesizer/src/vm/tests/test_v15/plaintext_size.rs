...
/// Builds a VM advanced to V15 height and runs `check_program_plaintext_sizes`
/// against `program_text` using the V15 bit budget.
fn run_check_at_v15(program_text: &str) -> Result<()> {
...
/// The worst-case payload from the threat model: a triply-nested 2048x2048x2048 bool array.
/// Materializing this would OOM any validator; the check rejects it on the type AST without
/// sampling a single leaf.
#[test]
fn test_worst_case_function_input_rejected() {
    let result = run_check_at_v15(
        r"
program worst_case.aleo;
...
",
    ...
    let err = result.expect_err("worst-case type must be rejected");
...
/// This subsumes the legacy `check_future_argument_bit_size` at V15 with a tighter budget.
#[test]
